...
for idx, fname in enumerate(images):
    ...
    # subplot_images(img,undis_img,'original','undistorted')
    # plt.show()
    ...
    #warp the image top down
    binary, M, minV = warp(combined, src, dst)
    binary2, M, minV = warp(undis_img, src, dst)
    ...
    for window in range(nwindows):
        ...
        cv2.putText(result, 'Radius of Curvature is: ' + str(round(left_curverad, 3)) + '(m)', (50, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 2)
        cv2.putText(result, 'Vehicle is: ' + str(abs(round(center_diff, 3))) + ' meter ' + side_pos + ' of center',
                    (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 2)

    # plt.imshow(result)
    # plt.show()
    write_to_fie = './imgs_tracked/test_tracked'+str(idx)+'.jpg'
    cv2.imwrite(write_to_fie,result)
